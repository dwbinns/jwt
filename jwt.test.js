import { create, importJWK, importPem, parse, verify } from "@dwbinns/jwt";
...
test("RS256", async () => {
    ...
    let rsaKey = await importJWK("RS256", undefined, rsaJWK);
    ...
    deepEqual({ alg: "RS256" }, header);
    deepEqual(exampleClaims, claims);

    assert(await verify(rsaKey, rsaJWT))
    assert(await verify(rsaKey, await create(rsaKey, claims)))
...
test("ES256", async () => {
    ...
    deepEqual(await verify(ecKey, ecJWT), exampleClaims);

    deepEqual(await verify(ecKey, await create(ecKey, claims)), exampleClaims);
